	...
	<view class="page">
		...
		<view v-for="one in list" :key="one.date">
			...
			<view class="item" v-for="meeting in one.list" :key="meeting.id"
				...
				<view class="header">
					<view class="left">
						<image v-if="meeting.type=='线上会议'" src="../../static/icon-11.png" mode="widthFix" class="icon">
						</image>
						<image v-if="meeting.type=='线下会议'" src="../../static/icon-11.png" mode="widthFix" class="icon">
						</image>
		...
		methods: {
			...
			enter: function(id, uuid, date, start) {
				...
				if (now.getTime() >= begin.getTime() - 10 * 60 * 1000) {
					this.ajax(this.url.searchRoomIdByUUID, "POST", {
						uuid,
						uuid
					}, function(resp) {
