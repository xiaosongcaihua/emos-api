<template>
	<view class="page" v-if="checkPermission(['ROOT', 'EMPLOYEE:SELECT'])">
		<view class="header">
			...
			<input type="text" confirm-type="search" v-model="keyword" @confirm="search" placeholder="输入要检索的员工姓名"
				maxlength="15" class="keyword" />
		...
		<view v-for="dept in list" :key="dept.id">
			...
			<uni-list v-if="dept.count > 0">
				<uni-list-item v-for="emp in dept.members" :title="emp.name" :key="emp.userId" link :to="'../user_info/user_info?userId=' + emp.userId+'?name='+emp.name">
				</uni-list-item> 
			</uni-list>
		</view>
	</view>
