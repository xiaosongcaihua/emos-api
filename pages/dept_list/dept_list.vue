	...
	<view class="page" v-if="checkPermission(permission, ['ROOT', 'DEPT:SELECT'])">
		<view class="list">
			<view class="item" v-for="one in list" :key="one.id" @longpress="deleteDept(one.id, one.deptName)">
				...
				<text class="value" @tap="updateDept(one.id, one.deptName)"
					v-if="checkPermission(permission, ['ROOT', 'DEPT:UPDATE'])">修改</text </view>
			</view>
			<button class="btn" @tap="insertDept"
				v-if="checkPermission(permission, ['ROOT', 'DEPT:INSERT'])">添加</button>
			<uni-popup ref="popupDept" type="dialog">
				<uni-popup-dialog mode="input" title="编辑部门名称" :value="deptName" placeholder="输入部门名称"
					@confirm="finishDept">
				</uni-popup-dialog>
			</uni-popup>
		</view>
